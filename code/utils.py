...
def prepare_maisi_controlnet_json_dataloader(
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root in zip(json_data_list, data_base_dir):
            with open(data_list) as f:
                json_data = json.load(f)[json_key]
    ...
    common_transform = [
        ...
        Lambdad(keys=["dino_feature_path"], func=lambda x: x, allow_missing_keys=True),
    ]
    train_transforms, val_transforms = Compose(common_transform), Compose(common_transform)

    train_loader = []  # 用空列表兜底，防止无数据时报错
    if len(list_train) > 0:
        if use_ddp and len(list_train) >= world_size:
            ...
            # 如果数据不够分，只让 Rank 0 跑，其他卡拿空列表
            list_train = list_train if rank == 0 else []

        if len(list_train) > 0:  # 再次检查分发后当前进程是否分到了数据
            train_ds = CacheDataset(data=list_train, transform=train_transforms, cache_rate=cache_rate, num_workers=8)
            train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True, drop_last=drop_last)

    val_loader = []  # 用空列表兜底
    if len(list_valid) > 0:
        if use_ddp and len(list_valid) >= world_size:
            list_valid = partition_dataset(
                data=list_valid,
                shuffle=False,             # 推理时不要打乱顺序
                num_partitions=world_size,
                even_divisible=False,      # 非常重要！保证不能整除的多余受试者不会被静默抛弃
            )[rank]
        elif use_ddp:
            list_valid = list_valid if rank == 0 else []
